...
async function createSaverRule(userId, dId, status) {
  ...
  try {
    const url = "http://"+process.env.HOST+":8085/api/v4/rules";
    console.log("URL " + url);
    ...
    console.log("Error creating device saver rule");
    console.log(error);
    return false;
  }
}
...
async function updateSaverRuleStatus(emqxRuleId, status) {
  try {
    const url = "http://"+process.env.HOST+":8085/api/v4/rules/" + emqxRuleId;
...
async function deleteSaverRule(dId) {
  try {
    ...
    const url = "http://"+process.env.HOST+":8085/api/v4/rules/" + mongoRule.emqxRuleId;
  ...
  try {
    ...
    if (rules.length > 0) {
      asyncForEach(rules, async rule => {
        const url = "http://"+process.env.HOST+":8085/api/v4/rules/" + rule.emqxRuleId;
