...
B4B_FFT::B4B_FFT(void)
{ // Constructor
	#warning("This method is deprecated and may be removed on future revisions.")
}

B4B_FFT::B4B_FFT(double *vReal, double *vImag, uint16_t samples, double samplingFrequency)
{// Constructor
	this->_vReal = vReal;
	this->_vImag = vImag;
...
uint8_t B4B_FFT::Revision(void)
{
	return(FFT_LIB_REV);
}

void B4B_FFT::Compute(double *vReal, double *vImag, uint16_t samples, uint8_t dir)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	Compute(vReal, vImag, samples, Exponent(samples), dir);
}

void B4B_FFT::Compute(uint8_t dir)
...
void B4B_FFT::Compute(double *vReal, double *vImag, uint16_t samples, uint8_t power, uint8_t dir)
{	// Computes in-place complex-to-complex FFT
	// Reverse bits
	#warning("This method is deprecated and may be removed on future revisions.")
	uint16_t j = 0;
...
void B4B_FFT::ComplexToMagnitude()
...
void B4B_FFT::ComplexToMagnitude(double *vReal, double *vImag, uint16_t samples)
{	// vM is half the size of vReal and vImag
	#warning("This method is deprecated and may be removed on future revisions.")
	for (uint16_t i = 0; i < samples; i++) {
...
void B4B_FFT::DCRemoval(double *vData, uint16_t samples)
{
	// calculate the mean of vData
	#warning("This method is deprecated and may be removed on future revisions.")
	double mean = 0;
...
void B4B_FFT::Windowing(uint8_t windowType, uint8_t dir)
...
void B4B_FFT::Windowing(double *vData, uint16_t samples, uint8_t windowType, uint8_t dir)
{// Weighing factors are computed once before multiple use of FFT
// The weighing function is symetric; half the weighs are recorded
	#warning("This method is deprecated and may be removed on future revisions.")
...
void B4B_FFT::MajorPeak(double *f, double *v)
{
	double maxY = 0;
	uint16_t IndexOfMaxY = 0;
	//If sampling_frequency = 2 * max_frequency in signal,
...
double B4B_FFT::MajorPeak(double *vD, uint16_t samples, double samplingFrequency)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	double maxY = 0;
	uint16_t IndexOfMaxY = 0;
...
void B4B_FFT::MajorPeak(double *vD, uint16_t samples, double samplingFrequency, double *f, double *v)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	double maxY = 0;
	uint16_t IndexOfMaxY = 0;
...
uint8_t B4B_FFT::Exponent(uint16_t value)
{
...
void B4B_FFT::Swap(double *x, double *y)
{
	double temp = *x;
	*x = *y;
	*y = temp;
